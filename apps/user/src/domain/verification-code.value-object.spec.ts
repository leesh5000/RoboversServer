...
describe('VerificationCode', () => {
  describe('generate', () => {
    ...
    it('should set expiration time to 5 minutes from now', () => {
      ...
      expect(expiresAt.getTime()).toBeGreaterThanOrEqual(expectedExpiration.getTime());
      expect(expiresAt.getTime()).toBeLessThanOrEqual(afterGenerate.getTime() + 5 * 60 * 1000);
    });

    it('should generate different codes', () => {
      const codes = new Set<string>();
      for (let i = 0; i < 100; i++) {
        codes.add(VerificationCode.generate().getValue());
      }
  ...
  describe('fromValue', () => {
    it('should create from valid code and expiration', () => {
      ...
      const code = VerificationCode.fromValue(codeValue, expiresAt);
    ...
    it('should throw error for invalid code length', () => {
      ...
      expect(() => VerificationCode.fromValue('12345', expiresAt))
        .toThrow('인증 코드는 6자리여야 합니다');
      expect(() => VerificationCode.fromValue('1234567', expiresAt))
        .toThrow('인증 코드는 6자리여야 합니다');
      expect(() => VerificationCode.fromValue('', expiresAt))
        .toThrow('인증 코드는 6자리여야 합니다');
    });
    ...
    it('should throw error for non-numeric code', () => {
      ...
      expect(() => VerificationCode.fromValue('12345a', expiresAt))
        .toThrow('인증 코드는 숫자로만 구성되어야 합니다');
      expect(() => VerificationCode.fromValue('abcdef', expiresAt))
        .toThrow('인증 코드는 숫자로만 구성되어야 합니다');
    });
  });
  ...
  describe('isExpired', () => {
    ...
    it('should return true for expired code', () => {
      ...
      const code = VerificationCode.fromValue('123456', pastDate);
  ...
  describe('matches', () => {
    it('should return true for matching code', () => {
      const codeValue = '123456';
      const code = VerificationCode.fromValue(codeValue, new Date());
      
      expect(code.matches(codeValue)).toBe(true);
    });

    it('should return false for non-matching code', () => {
      const code = VerificationCode.fromValue('123456', new Date());
